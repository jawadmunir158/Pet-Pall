...
const Ai = () => {
  ...
  return (
    <div className={`h-screen flex justify-center items-center`}>
      <div className={`max-w-2xl w-full rounded-lg shadow-lg p-6 flex flex-col space-y-4 ${darkMode ? 'bg-gray-800 text-white' : 'bg-transparent text-black'}`}>

        <div className='text-xl text-center pt-4 border-t mb-4'>
          <Title text1={'Assistant '} text2={'for Pets'} />
        </div>
        ...
        <textarea
          ...
          className={`w-full p-4 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${darkMode ? 'bg-gray-600 text-white border-gray-500' : 'bg-white text-black'}`}
          rows="3"
        ...
        <div className="flex justify-between items-center">
          <button
            ...
            className={`w-full ${darkMode ? 'bg-white text-black' : 'bg-black text-white'} hover:bg-gray-700 font-semibold py-2 px-4 rounded-lg ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          <FontAwesomeIcon
            icon={faMicrophone}
            size="2x"
            style={{ color: darkMode ? 'white' : 'black' }} // Set icon color directly
