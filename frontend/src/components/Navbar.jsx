...
const Navbar = () => {
  ...
  // Dynamically set logo based on dark mode
  const logoSrc = darkMode ? assets.logodark : assets.logo;

  return (
    <div>
      <div 
        className={`flex items-center justify-between py-5 font-medium ${darkMode ? 'bg-black text-white' : 'bg-white text-gray-700'} 
          ${scrollingDown ? 'fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out py-3 px-6' : 'relative py-5 px-6'}`
        }>
        <Link to='/'>
  ...
  <img src={assets.search_icon} className='w-5 cursor-pointer' alt="Search Icon" />
          ...
          <div className='group relative'>
            ...
            <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4'>
              <div className='flex flex-col gap-2 w-36 py-3 px-5 bg-slate-100 text-gray-500 rounded'>
                ...
                <p onClick={()=>navigate('/orders')} className='cursor-pointer hover:text-black'>Orders & Appointments</p>
      ...
      <div className={`absolute top-0 right-0 bottom-0 overflow-hidden ${darkMode ? 'bg-black text-white' : 'bg-white text-gray-600'} transition-all duration-300 ease-in-out ${visible ? 'w-full' : 'w-0'}`}>
